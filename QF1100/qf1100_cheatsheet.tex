...
\usepackage[landscape,a4paper,top=3mm,bottom=3mm,left=3mm,right=3mm]{geometry}
\usepackage{multicol}
...
\begin{itemize}
  \item Constant \(\delta\): \(a(t)=e^{\delta t}\), so \(\delta=\ln(1+r_e)\)
  \item Convert: \(1+r_e=e^{\delta}\Rightarrow r_e=e^{\delta}-1\)
\end{itemize}
...
Payments grow at rate $g$ per period, $n$ periods, first payment $A$ at $t=1$:
\[\mathrm{PV}=\frac{A}{r-g}\Bigl[1-\Bigl(\frac{1+g}{1+r}\Bigr)^n\Bigr]\quad(r\neq g)\]
If $r=g$: $\mathrm{PV}=\frac{nA}{1+r}$.\quad Growing perpetuity ($r>g$): $\mathrm{PV}=\dfrac{A}{r-g}$.

\subsection{Internal Rate of Return (IRR)}
IRR is $r$ s.t.\ $\mathrm{PV}(C)=0$.

\textbf{Uniqueness theorem:} If $c_0<0$, $c_k\ge0\;\forall k\ge1$, and $\sum c_k>0$, then IRR is unique.

Newton--Raphson: $r_{n+1}=r_n - \dfrac{f(r_n)}{f'(r_n)}$

\subsection{Loans \& Refinancing}
Loan \(L\) repaid by \(n\) equal payments \(A\) at rate \(r\) per period:
\[L=\frac{A}{r}\Bigl[1-\frac{1}{(1+r)^n}\Bigr]\quad\Rightarrow\quad A=\frac{Lr}{1-(1+r)^{-n}}\]

\textbf{Loan balance after \(m\) payments} (prospective):
\[L_m^{\text{bal}}=\frac{A}{r}\Bigl[1-\frac{1}{(1+r)^{n-m}}\Bigr]\]

\textbf{Retrospective method:}
\[L_m^{\text{bal}}=L(1+r)^m - \frac{A}{r}\bigl[(1+r)^m-1\bigr]\]

\begin{kbox}
Both methods give the same answer. Prospective is usually faster. Retrospective is useful when \(n\) is not given explicitly.
\end{kbox}
...
\textbf{Premium/discount form} (when \(F=R\)):
\[P=F+F\cdot\frac{c-\lambda}{\lambda}\Bigl[1-\frac{1}{(1+\lambda/m)^n}\Bigr]\]

\begin{itemize}
  \item \(c>\lambda\Rightarrow P>F\) (at premium)
  \item \(c=\lambda\Rightarrow P=F\) (at par)
  \item \(c<\lambda\Rightarrow P<F\) (at discount)
\end{itemize}
...
\textbf{Special cases:}
\begin{itemize}
  \item Zero-coupon bond: \(D=T\) (maturity)
  \item Perpetuity (\(m=1\)): \(D=\dfrac{1+\lambda}{\lambda}\)
  \item Coupon bond at par (\(P=F=R\), annual): \(D=\dfrac{1+\lambda}{\lambda}\Bigl[1-\frac{1}{(1+\lambda)^n}\Bigr]\)
\end{itemize}

Properties: \(t_1\le D\le t_n\). Duration independent of \(F\).
